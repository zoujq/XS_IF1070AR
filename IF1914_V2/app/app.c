...
extern uint8_t *get_adv_name();
extern uint8_t get_adv_name_len();
extern uint8_t *get_rsp_name();
extern uint8_t get_rsp_name_len();
/*
 * TYPE DEFINITIONS
 ****************************************************************************************
 */
...
void appm_init()
{
	...
	app_env.dev_name_len = get_adv_name_len();
    memcpy(&app_env.dev_name[0], get_adv_name(), get_adv_name_len());
...
{	
    ...
    {				
...
#endif                         
       //      {
       //          device_name_length = get_adv_name_len();
       //          // Get default Device Name (No name if not enough space)
       //          memcpy(&device_name_temp_buf[0], get_adv_name(), device_name_length);
       //      }
       ...
       //      }
        ...
        {
            extern uint8_t* get_manufacturer_data();
            extern uint8_t get_manufacturer_data_len();

            memcpy( cmd->info.host.adv_data+cmd->info.host.adv_data_len,get_manufacturer_data(),get_manufacturer_data_len());
            cmd->info.host.adv_data_len+=get_manufacturer_data_len();
        }
#if (NVDS_SUPPORT)
        // Scan Response Data
...
#endif                    
        {
			...
			memcpy(&cmd->info.host.scan_rsp_data[cmd->info.host.scan_rsp_data_len],
                   get_rsp_name(), get_rsp_name_len());
            cmd->info.host.scan_rsp_data_len += get_rsp_name_len();
        }
